...
class LMMDLoss(MMDLoss, LambdaSheduler):
    ...
    def cal_weight(self, source_label, target_label):
        batch_size = source_label.size(0)
        num_centers = target_label.size(0)
        ...
        target_label_onehot = np.eye(self.num_class)[target_label]
        target_label_sum = np.sum(target_label_onehot, axis=0).reshape(1, self.num_class)
        target_label_sum[target_label_sum == 0] = 100
        target_label_onehot = target_label_onehot / target_label_sum
        ...
        weight_tt = np.zeros((num_centers, num_centers))
        weight_st = np.zeros((batch_size, num_centers))
        ...
        for i in range(self.num_class): # (B, C)
            if i in set_s and i in set_t:
                ...
                t_tvec = target_label_onehot[:, i].reshape(num_centers, -1) # (B, 1)

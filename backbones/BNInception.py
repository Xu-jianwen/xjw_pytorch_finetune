...
def bninception(pretrained=True, embedding=False, dim=256, num_classes=3):
    model = BNInception(dim=dim, num_classes=num_classes)
    if pretrained:
        model.load_param("/home/xjw/jianwen/checkpoints/bn_inception-52deb4733.pth")
    return model


if __name__ == "__main__":
    bnmodel = bninception(dim=512, num_classes=28)
    input = torch.ones((1, 3, 64, 64))
    ft, output = bnmodel(input)
    print(output)

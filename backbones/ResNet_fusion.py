...
class ResNet(nn.Module):
    def __init__(self, block, layers, dim=512, num_classes=3, embedding=True):
        self.inplanes = 64
        ...
        self.embeddings = nn.Sequential(
            nn.Linear(3 * 512 * block.expansion, dim),
            nn.LeakyReLU(inplace=True),
        )
        self.embedding = embedding
        self.reduce1 = nn.Linear(25088, 2048)
        self.reduce2 = nn.Linear(4608, 2048)
        if self.embedding:
    ...
    def forward(self, x):
        ...
        # )
        ft1 = nn.MaxPool2d(7, stride=1)(x)
        ft1 = ft1.view(x.size(0), -1)
        ft2 = nn.AvgPool2d(2, stride=2)(x)
        ft2 = nn.MaxPool2d(3, stride=1)(ft2)
        ft2 = ft2.view(x.size(0), -1)
        x = self.avgpool(x)
        ft3 = x.view(x.size(0), -1)
...
def resnet50_ff(pretrained, embedding=False, dim=256, num_classes=3):
    model = ResNet(
        Bottleneck,
        [3, 4, 6, 3],
        embedding=embedding,
...
if __name__ == "__main__":
    model = resnet50_ff(pretrained=False, embedding=True, dim=512, num_classes=23)
